...
add_action('init', function () {
    if (strstr($_SERVER['REQUEST_URI'], get_option('slim_base_path','slim/api/'))) {
        ...
        $slim->post('/slim/api/appointment', function(){
            ...
            "inquirydetails(inquiryId, fullname, email, phoneNo, spices) VALUES(%s, %s, %s, %s, %s)";
            $stmt = $wpdb->prepare($sSQL,
                array($oInquiry->id, $oInquiry->fullname, $oInquiry->email, $oInquiry->phoneno, $oInquiry->spices ));
            $wpdb->query($stmt);
